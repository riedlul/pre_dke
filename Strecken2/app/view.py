...
# NEU seit Abschlussgespräch: Bahnhöfe in Abschnitten können nunmehr geändert werden:
@view.route('/bearbeitenBahnhofAbschnitt/<int:abschnittID>', methods=['GET', 'POST'])
def bearbeitenBahnhofAbschnitt(abschnittID):
    abschnittBearbeiten = AbschnittModel.query.get(abschnittID)
    abschnitte = AbschnittModel.query.all()
    bahnhöfe = BahnhofModel.query.all()
    ...
    if request.method == 'POST':
        ...
        neuerAbschnitt = AbschnittModel(start_id=a_start, end_id=a_end, spurweite=abschnittBearbeiten.spurweite, entgelt=abschnittBearbeiten.entgelt,
                                            lang=abschnittBearbeiten.lang, maxgesch=abschnittBearbeiten.maxgesch, abschnitt_warnung=abschnittBearbeiten.abschnitt_warnung)
        db.session.add(neuerAbschnitt)
        db.session.commit()
        db.session.delete(abschnittBearbeiten)
        db.session.commit()
    ...
    return render_template('bearbeitenBahnhofAbschnitt.html', title='Bahnhof in Abschnitte bearbeiten', user=current_user, abschnitte=abschnitte,
                           bahnhöfe=bahnhöfe)
...
def getStrecken():
    if request.method == 'POST':
        ...
        #Check ob Spurweiten übereinstimmen:
        for rs in rou_sections:
            if AbschnittModel.query.get(rou_sections[j]).spurweite != AbschnittModel.query.get(rou_sections[x]).spurweite:
                flash('Spurweiten stimmen nicht überein!', category='error')
                return redirect('/strecken')
            j+1
            x+1
        ...
        if len(rou_sections) >= 3:
            ...
            if len(list) != 1 or len(list2) != 1:
                ...
                neueStrecke = StreckenModel(name=name, strecken_abschnitt=abschnitte, startbahnhof="", endbahnhof="")
                db.session.add(neueStrecke)
                db.session.commit()
                flash('Strecke hinzugefügt!', category='success')
